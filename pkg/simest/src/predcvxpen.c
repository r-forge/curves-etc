	...
	for(int j = (k-1); j >= 0; j--){
		...
		for(int i = 0; i <= (n-2); i++){
			...
			if(flag != 1 && x[j] > L[i] && x[j] <= U[i]){
				...
				Q[j] = Q[j] - tmp*(L[i] - t[i])*(L[i] - t[i])*(x[j] - L[i]);
				tmp = fun[i]/(2*(t[i+1] - t[i]));
				Q[j] = Q[j] + tmp*(t[i+1] - L[i])*(t[i+1] - L[i])*(x[j] - L[i]);
			...
			if(flag != 1 && x[j] > U[i] && x[j] <= t[i+1]){
				...
				Q[j] = zhat[i] + deriv[i]*(x[j] - t[i]);
				tmp = fun[i+1]/(2*(t[i+1] - t[i]));
				Q[j] = Q[j] + tmp*((U[i] - t[i])*(U[i] - t[i]) - (L[i] - t[i])*(L[i] - t[i]))*(x[j] - U[i]);
